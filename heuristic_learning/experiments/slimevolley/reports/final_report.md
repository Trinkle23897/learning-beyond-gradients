...
- Additional generation-5 mixed direction: phase-gated opponent-side low pressure improved some hard archived rows, with `phase_two_frame` moving built-in by `+0.04` and `improved-v5/v6` by `+0.10/+0.12`; a later selective `phase_two_frame_descending` full-pool check improved built-in by `+0.04`, `improved-v3` by `+0.02`, and `improved-v5/v6` by `+0.08/+0.10`, but it regressed `improved-v2` by `-0.10` and stayed far below `baseline-rnn`, so no maintained policy edit was promoted.
